...
Pqr::Pqr(const string& fname) {
  struct stat st;
  int ret = stat(fname.c_str(), &st);
  if (ret != 0) {
    cerr << "The file \"" << fname << "\" does not exist: File_name = " << __FILE__ << " Line = " << __LINE__  << endl;
    exit(1);
  }
...
string Pqr::get_atom_name(const int& atom_id) {
  return m_PQR_atom_name[atom_id];
}

string Pqr::get_residue_name(const int& atom_id) {
...
vector<double> Pqr::center_of_mass() {
  vector<double> center(3, 0.0);
  int n = 0;

  for (int i = 0; i < (int)m_PQR_atom_x.size(); ++i) {
...
double Pqr::get_min_x() {
  ...
  for (int i = 1; i < (int)m_PQR_atom_x.size(); ++i) {
...
double Pqr::get_min_y() {
  ...
  for (int i = 1; i < (int)m_PQR_atom_y.size(); ++i) {
...
double Pqr::get_min_z() {
  ...
  for (int i = 1; i < (int)m_PQR_atom_z.size(); ++i) {
