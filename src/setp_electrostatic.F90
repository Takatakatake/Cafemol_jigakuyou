...
subroutine setp_electrostatic()
    ...
    use var_setp, only: inele, ingb, inmisc
...
!        if(mod(inmisc%itype_nlocal(iunit, junit), INTERACT%ELE) == 0) then
        ...
        loop_line: do iline = 1, nlines
           ...
           write (lunout, '(2a, i10, g10.3)') '---reading CHARGE_CHANGE: ', header, read_charge_change_imp, read_charge_change_value
                ...
                imp = read_charge_change_imp
                ingb%charge(imp) = read_charge_change_value
            end if
        ...
        if (any(inele%flag_ele)) then
            ...
            enddo
            write (lunout, '(a,1x,a,1x)', advance='NO') 'CHARGE_TYPE', 'OTH'
            write (lunout, '(f10.5)') inele%coef_charge_type(CHARGETYPE%OTH)
            write (lunout, '(a,1x,a,1x)', advance='NO') 'CHARGE_TYPE', 'P  '
            write (lunout, '(f10.5)') inele%coef_charge_type(CHARGETYPE%P)
            write (lunout, '(a,1x,a,1x)', advance='NO') 'CHARGE_TYPE', 'P2 '
            write (lunout, '(f10.5)') inele%coef_charge_type(CHARGETYPE%P2)
            write (lunout, '(a)') '>>>>'
        endif
