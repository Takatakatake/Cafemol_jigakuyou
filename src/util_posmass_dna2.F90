...
subroutine util_posmass_dna2(nunit, &  ![i ]
    ...
    use const_index
    use const_physical
    use var_struct, only: lunit2mp, iclass_unit, &
        cmp2seq, imp2type
...
!        write(*,*) 'posmass: imp=', imp
            ...
            if (iatomnum(imp) == 3) then
                !added error messsage as is in util_possmass.F90

                if (imp - 2 < lunit2mp(1, iunit) .or. imp + 1 > lunit2mp(2, iunit)) then
                    !represent initial coordinate of phospate group by P atom if Phosphate is at the terminal of chain
                    xyz_mp_init(1:3, imp) = xyz(1:3, 1, imp)   ! P

                    if (inmisc%i_go_atom_dna == 0) then
                        write (error_message, '(a,a,i5,a,i5,a,i5)') "Warning: ", &
                     "terminal phosphate is incomplete. Initial coordiante of phospahte group is represented by P atom at iunit=", &
                                            iunit," imp=",imp ," instead of center-of-mass of phosphate. This may gives inappropriate go_nat for interchain Go-contacts involving imp =",imp
                        call util_error(ERROR%WARN_ALL, error_message)

                    else

                        write (error_message, '(a,a,i5,a,i5,a)') "Warning: ", &
                     "terminal phosphate is incomplete. Initial coordiante of phospahte group is represented by P atom at iunit=", &
                            iunit, " imp=", imp, " instead of center-of-mass of phosphate."
                        call util_error(ERROR%WARN_ALL, error_message)
                    end if

                else
                    ! calc of xyz_mp_init(1:3,imp) using P, OP1, OP2, O5' and O3'
