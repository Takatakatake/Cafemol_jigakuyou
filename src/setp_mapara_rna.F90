...
subroutine setp_mapara_rna(lunpara, lunout)
    ...
    use const_index
    use const_physical
    use var_setp, only: inrna, indtrna, inarna
#ifdef MPI_PAR
    use mpiconst
#endif
    ...
    indtrna%energy_unit = INVALID_VALUE
    indtrna%i_use_atom_base = -1
    indtrna%i_use_atom_sugar = -1
    indtrna%cbd_PS = INVALID_VALUE
    indtrna%cbd_SB = INVALID_VALUE
    indtrna%cbd_SP = INVALID_VALUE
    indtrna%cba_PSB = INVALID_VALUE
    indtrna%cba_PSP = INVALID_VALUE
    indtrna%cba_BSP = INVALID_VALUE
    indtrna%cba_SPS = INVALID_VALUE
    indtrna%cdist_exvwca = INVALID_VALUE
    indtrna%coef_exvwca = INVALID_VALUE
    indtrna%n_sep_nlocal_P = -1
    indtrna%n_sep_nlocal_S = -1
    indtrna%n_sep_nlocal_B = -1
    indtrna%cst_dist = INVALID_VALUE
    indtrna%cst_dih = INVALID_VALUE
    indtrna%cst_h(1:16) = INVALID_VALUE
    indtrna%cst_s(1:16) = INVALID_VALUE
    indtrna%cst_Tm(1:16) = INVALID_VALUE
    indtrna%chb_dist = INVALID_VALUE
    indtrna%chb_angl = INVALID_VALUE
    indtrna%chb_dih_hbond = INVALID_VALUE
    indtrna%chb_dih_chain = INVALID_VALUE
    indtrna%chb_u0 = INVALID_VALUE
    ...
    if (myrank == 0) then
        ...
        ! -------------------------------------------------------------------
        rewind (lunpara)
        call ukoto_uiread2(lunpara, lunout, 'para_DT_rna     ', kfind, &
                           CARRAY_MXLINE, nlines, cwkinp)

        if (kfind /= 'FIND') then
            error_message = 'Error: cannot find "para_DT_rna" in the rna.para file'
            call util_error(ERROR%STOP_ALL, error_message)
        end if
        ...
        do iline = 1, nlines
            ...
            do iequat = 1, nequat
                cvalue = 'energy_unit_rna'
                call ukoto_rvalue2(lunout, csides(1, iequat), &
                                   indtrna%energy_unit, cvalue)

                cvalue = 'i_use_atom_base'
                call ukoto_ivalue2(lunout, csides(1, iequat), &
                                   indtrna%i_use_atom_base, cvalue)

                cvalue = 'i_use_atom_sugar'
                call ukoto_ivalue2(lunout, csides(1, iequat), &
                                   indtrna%i_use_atom_sugar, cvalue)

                cvalue = 'cbd_PS'
                call ukoto_rvalue2(lunout, csides(1, iequat), &
                                   indtrna%cbd_PS, cvalue)

                cvalue = 'cbd_SB'
                call ukoto_rvalue2(lunout, csides(1, iequat), &
                                   indtrna%cbd_SB, cvalue)

                cvalue = 'cbd_SP'
                call ukoto_rvalue2(lunout, csides(1, iequat), &
                                   indtrna%cbd_SP, cvalue)

                cvalue = 'cba_PSB'
                call ukoto_rvalue2(lunout, csides(1, iequat), &
                                   indtrna%cba_PSB, cvalue)

                cvalue = 'cba_PSP'
                call ukoto_rvalue2(lunout, csides(1, iequat), &
                                   indtrna%cba_PSP, cvalue)

                cvalue = 'cba_BSP'
                call ukoto_rvalue2(lunout, csides(1, iequat), &
                                   indtrna%cba_BSP, cvalue)

                cvalue = 'cba_SPS'
                call ukoto_rvalue2(lunout, csides(1, iequat), &
                                   indtrna%cba_SPS, cvalue)

                cvalue = 'cdist_exvwca'
                call ukoto_rvalue2(lunout, csides(1, iequat), &
                                   indtrna%cdist_exvwca, cvalue)

                cvalue = 'coef_exvwca'
                call ukoto_rvalue2(lunout, csides(1, iequat), &
                                   indtrna%coef_exvwca, cvalue)

                cvalue = 'n_sep_nlocal_P'
                call ukoto_ivalue2(lunout, csides(1, iequat), &
                                   indtrna%n_sep_nlocal_P, cvalue)

                cvalue = 'n_sep_nlocal_S'
                call ukoto_ivalue2(lunout, csides(1, iequat), &
                                   indtrna%n_sep_nlocal_S, cvalue)

                cvalue = 'n_sep_nlocal_B'
                call ukoto_ivalue2(lunout, csides(1, iequat), &
                                   indtrna%n_sep_nlocal_B, cvalue)

                cvalue = 'cst_dist'
                call ukoto_rvalue2(lunout, csides(1, iequat), &
                                   indtrna%cst_dist, cvalue)

                cvalue = 'cst_dih'
                call ukoto_rvalue2(lunout, csides(1, iequat), &
                                   indtrna%cst_dih, cvalue)

                cvalue = 'chb_dist'
                call ukoto_rvalue2(lunout, csides(1, iequat), &
                                   indtrna%chb_dist, cvalue)

                cvalue = 'chb_angl'
                call ukoto_rvalue2(lunout, csides(1, iequat), &
                                   indtrna%chb_angl, cvalue)

                cvalue = 'chb_dih_hbond'
                call ukoto_rvalue2(lunout, csides(1, iequat), &
                                   indtrna%chb_dih_hbond, cvalue)

                cvalue = 'chb_dih_chain'
                call ukoto_rvalue2(lunout, csides(1, iequat), &
                                   indtrna%chb_dih_chain, cvalue)

                cvalue = 'chb_u0'
                call ukoto_rvalue2(lunout, csides(1, iequat), &
                                   indtrna%chb_u0, cvalue)
            end do
        end do

        if (indtrna%energy_unit > INVALID_JUDGE) then
            error_message = 'Error: invalid value for indtrna%energy_unit'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%i_use_atom_base < 0) then
            error_message = 'Error: invalid value for indtrna%i_use_atom_base'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%i_use_atom_sugar < 0) then
            error_message = 'Error: invalid value for indtrna%i_use_atom_sugar'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%cbd_PS > INVALID_JUDGE) then
            error_message = 'Error: invalid value for indtrna%cbd_PS'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%cbd_SB > INVALID_JUDGE) then
            error_message = 'Error: invalid value for indtrna%cbd_SB'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%cbd_SP > INVALID_JUDGE) then
            error_message = 'Error: invalid value for indtrna%cbd_SP'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%cba_PSB > INVALID_JUDGE) then
            error_message = 'Error: invalid value for indtrna%cba_PSB'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%cba_PSP > INVALID_JUDGE) then
            error_message = 'Error: invalid value for indtrna%cba_PSP'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%cba_BSP > INVALID_JUDGE) then
            error_message = 'Error: invalid value for indtrna%cba_BSP'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%cba_SPS > INVALID_JUDGE) then
            error_message = 'Error: invalid value for indtrna%cba_SPS'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%cdist_exvwca > INVALID_JUDGE) then
            error_message = 'Error: invalid value for indtrna%cdist_exvwca'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%coef_exvwca > INVALID_JUDGE) then
            error_message = 'Error: invalid value for indtrna%coef_exvwca'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%n_sep_nlocal_P < 0) then
            error_message = 'Error: invalid value for indtrna%n_sep_nlocal_P'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%n_sep_nlocal_S < 0) then
            error_message = 'Error: invalid value for indtrna%n_sep_nlocal_S'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%n_sep_nlocal_B < 0) then
            error_message = 'Error: invalid value for indtrna%n_sep_nlocal_B'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%cst_dist > INVALID_JUDGE) then
            error_message = 'Error: invalid value for indtrna%cst_dist'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%cst_dih > INVALID_JUDGE) then
            error_message = 'Error: invalid value for indtrna%cst_dih'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%chb_dist > INVALID_JUDGE) then
            error_message = 'Error: invalid value for indtrna%chb_dist'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%chb_angl > INVALID_JUDGE) then
            error_message = 'Error: invalid value for indtrna%chb_angl'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%chb_dih_hbond > INVALID_JUDGE) then
            error_message = 'Error: invalid value for indtrna%chb_dih_hbond'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%chb_dih_chain > INVALID_JUDGE) then
            error_message = 'Error: invalid value for indtrna%chb_dih_chain'
            call util_error(ERROR%STOP_ALL, error_message)

        elseif (indtrna%chb_u0 > INVALID_JUDGE) then
            error_message = 'Error: invalid value for indtrna%chb_u0'
            call util_error(ERROR%STOP_ALL, error_message)
        ...
        if (indtrna%i_use_atom_base == USE_RNA_BASE%COM) then
            write (lunout, *) 'using the center of mass for base in DT_RNA'
        else if (indtrna%i_use_atom_base == USE_RNA_BASE%PuN1_PyN3) then
        ...
        if (indtrna%i_use_atom_sugar == USE_RNA_SUGAR%COM) then
            write (lunout, *) 'using the center of mass for sugar in DT_RNA'
        else if (indtrna%i_use_atom_sugar == USE_RNA_SUGAR%COM_RING) then
            write (lunout, *) 'using the center of mass of sugar-ring position for sugar in DT_RNA'
        else if (indtrna%i_use_atom_sugar == USE_RNA_SUGAR%C4) then
        ...
        call ukoto_uiread2(lunpara, lunout, 'DT_stack_param  ', kfind, &
                           CARRAY_MXLINE, nlines, cwkinp)

        if (kfind /= 'FIND') then
            error_message = 'Error: cannot find "DT_stack_param" in the rna.para file'
            call util_error(ERROR%STOP_ALL, error_message)
        end if
        ...
        do iline = 1, nlines
            ...
            indtrna%cst_h(itype) = param1   ! h
            indtrna%cst_s(itype) = param2   ! s
            indtrna%cst_Tm(itype) = param3  ! Tm

            write (lunout, '(a,a2,3(x1g10.3))') '---reading stack parameter: ', ctype, param1, param2, param3
        enddo

        do inn = 1, 16
            if (indtrna%cst_h(inn) > INVALID_JUDGE) then
                error_message = 'Error: invalid value for indtrna%cst_h'
                call util_error(ERROR%STOP_ALL, error_message)

            else if (indtrna%cst_s(inn) > INVALID_JUDGE) then
                error_message = 'Error: invalid value for indtrna%cst_h'
                call util_error(ERROR%STOP_ALL, error_message)

            else if (indtrna%cst_Tm(inn) > INVALID_JUDGE) then
                error_message = 'Error: invalid value for indtrna%cst_h'
                call util_error(ERROR%STOP_ALL, error_message)
            endif
        enddo
    ...
    call MPI_Bcast(indtrna, indtrna%sz, MPI_BYTE, 0, MPI_COMM_WORLD, ierr)
