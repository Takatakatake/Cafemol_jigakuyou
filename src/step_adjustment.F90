...
!
            call mpi_allgather(ele_list(:)%num_pairs, n_replica_mpi, MPI_INTEGER, &
...
!
            call mpi_allreduce(ele_list(:)%num_pairs, lele_inn, n_replica_mpi, MPI_INTEGER, &
...
#else
            lele_all(1:n_replica_mpi) = ele_list(1:n_replica_mpi)%num_pairs
#endif
            TIME_E(tmc_step_adj)
