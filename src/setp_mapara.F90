...
subroutine setp_mapara(lunpara, lunout)
    ...
    inpara%fric_scale = -1.0
    inpara%fric_typical_coef = -1.0
    inpara%cmass(0:CHEMICALTYPE%MAX) = -1.0
    inpara%stokes_radius(0:CHEMICALTYPE%MAX) = -1.0
    ...
    if (myrank == 0) then
        ...
        do iline = 1, nlines
            ...
            do iequa = 1, nequat
                ...
                cvalue = 'fric_scale'
                call ukoto_rvalue2(lunout, csides(1, iequa), &
                                   inpara%fric_scale, cvalue)

                cvalue = 'fric_typical_coef'
                call ukoto_rvalue2(lunout, csides(1, iequa), &
                                   inpara%fric_typical_coef, cvalue)
        ...
        else if (inpara%fric_scale < 0.0) then
            error_message = 'Error: invalid value for fric_scale'
            call util_error(ERROR%STOP_ALL, error_message)

        else if (inpara%fric_typical_coef < 0.0) then
            error_message = 'Error: invalid value for fric_typical_coef'
            call util_error(ERROR%STOP_ALL, error_message)
        ...
        do iline = 1, nlines
            ...
            if (cline(1:13) == 'STOKES_RADIUS') then
                write (ctype, '(100a)') (' ', i=1, 100)
                read (cline, *) cdummy, ctype, x
                write (lunout, '(2a,1x,1a,2x,f6.2)') '---reading Stokes radius: ', trim(cdummy), trim(ctype), x
                inpara%stokes_radius(char2ichem(ctype)) = x
            end if
        end do
