...
subroutine inp_filename()
    ...
    use var_inp, only: infile, outfile, i_run_mode, &
        ...
        fullpath, iopen_lunnum, ifile_out_opt, &
        ifile_out_muca, ifile_out_gro, ifile_out_top, &
        path
    ...
    integer :: imovie, icrd, ivelo, idcd, ivdcd, ipdb, ipsf, irst, iopt, imuca, igro, itop
    ...
    character(CARRAY_MXFILE) :: filename_muca
    character(CARRAY_MXFILE) :: filename_gro
    character(CARRAY_MXFILE) :: filename_top
    ! -------------------------------------------------------------------- mod
    character(4)  :: kfind
...
#else
    character(7), parameter :: FILE_STATUS = 'REPLACE'
#endif
    ...
    path = ""
    filename_header = ""
    filename_data = "./cafemol.data"
    filename_ninfo = "./cafemol.ninfo"
    filename_movie = "./cafemol.movie"
    filename_crd = "./cafemol.crd"
    filename_velo = "./cafemol.velo"
    filename_dcd = "./cafemol.dcd"
    filename_vdcd = "./cafemol.vdcd"
    filename_pdb = "./cafemol.pdb"
    filename_rep = "./cafemol.rep"
    filename_fmat = "./cafemol.fmat"
    filename_psf = "./cafemol.psf"
    filename_rst = "./cafemol.rst"
    filename_opt = "./cafemol.opt"
    filename_muca = "./cafemol.muca"
    filename_gro = "./cafemol.gro"
    filename_top = "./cafemol.top"
                ...
                do k = 1, CARRAY_MXCOLM
                    if (ctmp00(k:k) /= ' ' .and. ctmp00(k:k) /= ',') then
                        ...
                        if (char7 == 'crd') then
                            ...
                            iopt = 1
                        else if (char7 == 'muca') then
                            filename_muca = filename_header
                            filename_muca(n:n + 4) = '.muca'
                            imuca = 1
                        else if (char7 == 'gro') then
                            filename_gro = filename_header
                            filename_gro(n:n + 3) = '.gro'
                            igro = 1
                        else if (char7 == 'top') then
                            filename_top = filename_header
                            filename_top(n:n + 3) = '.top'
                            itop = 1
                        end if
...
! --------------------------------------------------------------------------
    ...
    call MPI_Bcast(filename_muca, CARRAY_MXFILE, MPI_CHARACTER, 0, MPI_COMM_WORLD, ierr)
    call MPI_Bcast(filename_gro, CARRAY_MXFILE, MPI_CHARACTER, 0, MPI_COMM_WORLD, ierr)
    call MPI_Bcast(filename_top, CARRAY_MXFILE, MPI_CHARACTER, 0, MPI_COMM_WORLD, ierr)
! --------------------------------------------------------------------------
    ...
    call MPI_Bcast(imuca, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)
    call MPI_Bcast(igro, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)
    call MPI_Bcast(itop, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)
#endif
...
#endif
                                        n = index(path, ' ')
                                        m = index(filename_muca, ' ')

                                        if (path /= '') then
                                            filename = path(1:n - 1)//'/'//filename_muca(1:m - 1)
                                        else
                                            filename = filename_muca
                                        end if

                                        write (*, '(a14,i3,a3,a)') "open muca file(", outfile%muca, "): ", trim(filename)
                                        open (outfile%muca, file=filename, status=FILE_STATUS, &
                                              action='write', iostat=iopen_status)
                                        if (iopen_status > 0) then
                                            error_message = 'Error: cannot open the file: '//filename
                                            call util_error(ERROR%STOP_STD, error_message)
                                ...
                                if (igro == 1) then
                                    ...
                                    if (local_rank_mpi == 0) then
#endif
                                        n = index(path, ' ')
                                        m = index(filename_gro, ' ')

                                        if (path /= '') then
                                            filename = path(1:n - 1)//'/'//filename_gro(1:m - 1)
                                        else
                                            filename = filename_gro
                                        end if

                                        filename_save = filename
                                        i_cend_save = index(filename_save, '.gro') - 1
                                        lunnum = iopen_lunnum
                                        iopen_lunnum = iopen_lunnum + n_replica_all
#ifdef MPI_REP
                                        jlen = (n_replica_all - 1 + npar_rep)/npar_rep
                                        ...
                                        do irep = jsta, jend
                                            ...
                                            do irep = 1, n_replica_all
#endif
                                                outfile%gro(irep) = lunnum
                                                lunnum = lunnum + 1

                                                if (flg_replica) then
                                                    write (crep, '(i100)') irep + n_zeroize
                                                    filename = filename_save(1:i_cend_save)//'_' &
                                                               //crep(101 - FILENAME_DIGIT_REPLICA:100)//'.gro'
                                                endif ! replica

                                               write (*, '(a14,i3,a3,a)') "open gro file(", outfile%gro(irep), "): ", trim(filename)
                                                open (outfile%gro(irep), file=filename, status=FILE_STATUS, &
                                                      action='write', iostat=iopen_status)
                                                if (iopen_status > 0) then
                                                    error_message = 'Error: cannot open the file: '//filename
                                                    call util_error(ERROR%STOP_STD, error_message)
...
#endif
                                            n = index(path, ' ')
                                            m = index(filename_top, ' ')

                                            if (path /= '') then
                                                filename = path(1:n - 1)//'/'//filename_top(1:m - 1)
                                            else
                                                filename = filename_top
                                            end if

                                            write (*, '(a14,i3,a3,a)') "open top file(", outfile%top, "): ", trim(filename)
                                            open (outfile%top, file=filename, status=FILE_STATUS, &
                                                  action='write', iostat=iopen_status)
                                            if (iopen_status > 0) then
                                                error_message = 'Error: cannot open the file: '//filename
                                                call util_error(ERROR%STOP_STD, error_message)
                                    ...
                                    ! -----------------------------------------------------------------------
#ifdef _DUMP_COMMON
                                    open (outfile%dump, file='cafemol.dump', status=FILE_STATUS, action='write', &
                                          iostat=iopen_status)
                                    if (iopen_status > 0) then
                                        error_message = 'Error: cannot open cafemol.dump'
                                        call util_error(ERROR%STOP_STD, error_message)
                                    endif
#endif
                                            ...
                                            do irep = jsta, jend
                                                ...
                                                do irep = 1, n_replica_all
                                                    ...
                                                    write (*, '(a15,i3,a3,a)') "check rst file(", outfile%rst(irep), &
                                                        "): ", trim(fullpath%rst(irep))
                                                    open (outfile%rst(irep), file=fullpath%rst(irep), status=FILE_STATUS, &
                                                          action='write', iostat=iopen_status)
                                                    if (iopen_status > 0) then
                                                        error_message = 'Error: cannot open the file: '//trim(fullpath%rst(irep))
                                                        call util_error(ERROR%STOP_STD, error_message)
                                                    end if
