...
subroutine allocate_nativestruct()
    ...
    use var_struct, only: nmp_all, &
        ibd2mp, ibd2type, bd_nat, factor_bd, coef_bd, correct_bd_mgo, iba2mp, &
        iba2type, iunit2ba, ba_nat, factor_ba, coef_ba, correct_ba_mgo, &
        idih2mp, idih2type, iunit2dih, dih_nat, factor_dih, coef_dih, &
        dih_sin_nat, dih_cos_nat, correct_dih_mgo, icon2mp, icon2type, lmp2con, &
        icon2unit, icon_dummy_mgo, go_nat, go_nat2, factor_go, coef_go, &
        imorse2mp, imorse2type, lmp2morse, imorse2unit, imorse_dummy_mgo, &
        morse_nat, morse_nat2, factor_morse, coef_morse_fD, coef_morse_a, &
        irna_bp2mp, lmp2rna_bp, irna_bp2unit, nhb_bp, irna_bp_dummy_mgo, &
        rna_bp_nat, rna_bp_nat2, coef_rna_bp, coef_rna_bp_a, coef_rna_bp_fD, &
        factor_rna_bp, irna_st2mp, lmp2rna_st, irna_st2unit, irna_st_dummy_mgo, &
        rna_st_nat, rna_st_nat2, coef_rna_st, coef_rna_st_a, coef_rna_st_fD, &
        factor_rna_st, istangle2mp, stangle_nat, factor_stangle, coef_stangle, &
        ...
        ibp2mp_dna2, ibp2ebp_dna2, ibp2sbp_dna2, ibp2pbp_dna2, ibp2t1bp_dna2, &
        ibp2t2bp_dna2, & ! DNA (3SPN.2 model) Base pairing
        ibp2ecstk1_dna2, ibp2ecstk2_dna2, ibp2scstk1_dna2, ibp2scstk2_dna2, &
        ibp2tcstk1_dna2, ibp2tcstk2_dna2, ibp2t3cstk_dna2, & ! DNA (3SPN.2 model) Cross stacking
        idtrna_st2mp, idtrna_st2nn, dtrna_st_nat, coef_dtrna_st, idtrna_hb2mp, &
        dtrna_hb_nat, coef_dtrna_hb, &
        ipdpwm2pro_mp, pro_mp2pdpwm_para, pro_mp2pdpwm_r0_cutoff, & ! protein-DNA seq-specific interaction
        pdpwm_mp_N_ca, pdpwm_mp_C_ca, pdpwm_mp_5_base, pdpwm_mp_3_base, & ! protein-DNA seq-specific interaction
        pdpwm_para_r0, pdpwm_para_a530, pdpwm_para_aNC0, pdpwm_para_aSB0, & ! protein-DNA seq-specific interaction
        pdpwm_para_pwm_A, pdpwm_para_pwm_C, pdpwm_para_pwm_G, pdpwm_para_pwm_T, & ! protein-DNA seq-specific interaction
        pdpwm_local_coef, pdpwm_local_shift, & ! protein-DNA seq-specific interaction
        ipdns2pro_mp, pro_mp2pdns_para, pro_mp2pdns_r0_cutoff, & ! protein-DNA seq-non-specific interaction
        pdns_mp_N_ca, pdns_mp_C_ca, & ! protein-DNA seq-non-specific interaction
        pdns_para_r0, pdns_para_aNC0, pdns_para_aSB0, & ! protein-DNA seq-non-specific interaction
        pdns_coef, & ! protein-DNA seq-non-specific interaction
    ...
    ! protein-DNA sequence-specific interactions
    if (inmisc%force_flag(INTERACT%PRO_DNA_PWM)) then
        allocate (ipdpwm2pro_mp(nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        ipdpwm2pro_mp(:) = -1

        allocate (pro_mp2pdpwm_para(MXMPPDPWM, nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pro_mp2pdpwm_para(:, :) = 0

        allocate (pro_mp2pdpwm_r0_cutoff(nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pro_mp2pdpwm_r0_cutoff(:) = 0.0e0_PREC

        allocate (pdpwm_mp_N_ca(nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pdpwm_mp_N_ca(:) = 0

        allocate (pdpwm_mp_C_ca(nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pdpwm_mp_C_ca(:) = 0

        allocate (pdpwm_mp_5_base(nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pdpwm_mp_5_base(:) = 0

        allocate (pdpwm_mp_3_base(nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pdpwm_mp_3_base(:) = 0

        allocate (pdpwm_para_r0(MXMPPDPWM*nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pdpwm_para_r0(:) = 0.0e0_PREC

        allocate (pdpwm_para_aSB0(MXMPPDPWM*nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pdpwm_para_aSB0(:) = 0.0e0_PREC

        allocate (pdpwm_para_aNC0(MXMPPDPWM*nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pdpwm_para_aNC0(:) = 0.0e0_PREC

        allocate (pdpwm_para_a530(MXMPPDPWM*nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pdpwm_para_a530(:) = 0.0e0_PREC

        allocate (pdpwm_local_coef(MXMPPDPWM*nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pdpwm_local_coef(:) = 1.0e0_PREC

        allocate (pdpwm_local_shift(MXMPPDPWM*nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pdpwm_local_shift(:) = 0.0e0_PREC

        allocate (pdpwm_para_pwm_A(MXMPPDPWM*nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pdpwm_para_pwm_A(:) = 0.0e0_PREC

        allocate (pdpwm_para_pwm_C(MXMPPDPWM*nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pdpwm_para_pwm_C(:) = 0.0e0_PREC

        allocate (pdpwm_para_pwm_G(MXMPPDPWM*nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pdpwm_para_pwm_G(:) = 0.0e0_PREC

        allocate (pdpwm_para_pwm_T(MXMPPDPWM*nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pdpwm_para_pwm_T(:) = 0.0e0_PREC
    end if

    ! protein-DNA sequence-non-specific interactions
    if (inmisc%force_flag(INTERACT%PRO_DNA_NONSPEC)) then
        allocate (ipdns2pro_mp(nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        ipdns2pro_mp(:) = -1

        allocate (pro_mp2pdns_para(MXMPPDNS, nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pro_mp2pdns_para(:, :) = 0

        allocate (pro_mp2pdns_r0_cutoff(nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pro_mp2pdns_r0_cutoff(:) = 0.0e0_PREC

        allocate (pdns_mp_N_ca(nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pdns_mp_N_ca(:) = 0

        allocate (pdns_mp_C_ca(nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pdns_mp_C_ca(:) = 0

        allocate (pdns_para_r0(MXMPPDNS*nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pdns_para_r0(:) = 0.0e0_PREC

        allocate (pdns_para_aSB0(MXMPPDNS*nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pdns_para_aSB0(:) = 0.0e0_PREC

        allocate (pdns_para_aNC0(MXMPPDNS*nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pdns_para_aNC0(:) = 0.0e0_PREC

        allocate (pdns_coef(MXMPPDNS*nmp_all), stat=ier)
        if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
        pdns_coef(:) = -1.0e0_PREC
    end if
...
contains
    subroutine check()
        ...
        !   protein-DNA                         sequence-specific  interactions
        if (allocated(ipdpwm2pro_mp)) flg_error = .true.
        if (allocated(pro_mp2pdpwm_para)) flg_error = .true.
        if (allocated(pro_mp2pdpwm_r0_cutoff)) flg_error = .true.
        if (allocated(pdpwm_mp_N_ca)) flg_error = .true.
        if (allocated(pdpwm_mp_C_ca)) flg_error = .true.
        if (allocated(pdpwm_mp_5_base)) flg_error = .true.
        if (allocated(pdpwm_mp_3_base)) flg_error = .true.
        if (allocated(pdpwm_para_r0)) flg_error = .true.
        if (allocated(pdpwm_para_a530)) flg_error = .true.
        if (allocated(pdpwm_para_aNC0)) flg_error = .true.
        if (allocated(pdpwm_para_aSB0)) flg_error = .true.
        if (allocated(pdpwm_local_coef)) flg_error = .true.
        if (allocated(pdpwm_local_shift)) flg_error = .true.
        if (allocated(pdpwm_para_pwm_A)) flg_error = .true.
        if (allocated(pdpwm_para_pwm_C)) flg_error = .true.
        if (allocated(pdpwm_para_pwm_G)) flg_error = .true.
        if (allocated(pdpwm_para_pwm_T)) flg_error = .true.
        !   protein-DNA                         sequence-non-specific  interactions
        if (allocated(ipdns2pro_mp)) flg_error = .true.
        if (allocated(pro_mp2pdns_para)) flg_error = .true.
        if (allocated(pro_mp2pdns_r0_cutoff)) flg_error = .true.
        if (allocated(pdns_mp_N_ca)) flg_error = .true.
        if (allocated(pdns_mp_C_ca)) flg_error = .true.
        if (allocated(pdns_para_r0)) flg_error = .true.
        if (allocated(pdns_para_aNC0)) flg_error = .true.
        if (allocated(pdns_para_aSB0)) flg_error = .true.
        if (allocated(pdns_coef)) flg_error = .true.
...
subroutine deallocate_nativestruct
    ...
    use var_struct, only: ibd2mp, ibd2type, bd_nat, factor_bd, coef_bd, &
        correct_bd_mgo, iba2mp, iba2type, iunit2ba, ba_nat, factor_ba, coef_ba, &
        correct_ba_mgo, idih2mp, idih2type, iunit2dih, dih_nat, factor_dih, &
        coef_dih, dih_sin_nat, dih_cos_nat, correct_dih_mgo, icon2mp, icon2type, &
        lmp2con, icon2unit, icon_dummy_mgo, go_nat, go_nat2, factor_go, coef_go, &
        imorse2mp, imorse2type, lmp2morse, imorse2unit, imorse_dummy_mgo, &
        morse_nat, morse_nat2, factor_morse, coef_morse_fD, coef_morse_a, &
        irna_bp2mp, lmp2rna_bp, irna_bp2unit, nhb_bp, irna_bp_dummy_mgo, &
        rna_bp_nat, rna_bp_nat2, coef_rna_bp, coef_rna_bp_a, coef_rna_bp_fD, &
        ...
        para_sasa, rad_sasa, surf, connect, & !sasa
        istk2mp_dna2, istk2ebstk_dna2, istk2sbstk_dna2, istk2tbstk_dna2, & ! DNA (3SPN.2 model) Base stacking
        ibp2mp_dna2, ibp2ebp_dna2, ibp2sbp_dna2, ibp2pbp_dna2, ibp2t1bp_dna2, & ! DNA (3SPN.2 model) Base pairing
        ibp2t2bp_dna2, & ! DNA (3SPN.2 model) Base pairing
        ibp2ecstk1_dna2, ibp2ecstk2_dna2, ibp2scstk1_dna2, ibp2scstk2_dna2, & ! DNA (3SPN.2 model) Cross stacking
        ibp2tcstk1_dna2, ibp2tcstk2_dna2, ibp2t3cstk_dna2, & ! DNA (3SPN.2 model) Cross stacking
        ipdpwm2pro_mp, pro_mp2pdpwm_para, pro_mp2pdpwm_r0_cutoff, pdpwm_para_r0, & ! PDPWM
        pdpwm_mp_N_ca, pdpwm_mp_C_ca, pdpwm_mp_5_base, pdpwm_mp_3_base, & ! protein-DNA seq-specific interaction
        pdpwm_para_a530, pdpwm_para_aNC0, pdpwm_para_aSB0, pdpwm_para_pwm_A, & ! PDPWM
        pdpwm_para_pwm_C, pdpwm_para_pwm_G, pdpwm_para_pwm_T, & ! PDPWM
        pdpwm_local_coef, pdpwm_local_shift, & ! PDPWM
        ipdns2pro_mp, pro_mp2pdns_para, pro_mp2pdns_r0_cutoff, pdns_para_r0, & ! protein-DNA seq-non-specific interaction
        pdns_mp_N_ca, pdns_mp_C_ca, & ! protein-DNA seq-non-specific interaction
        pdns_para_aNC0, pdns_para_aSB0, pdns_coef, & ! protein-DNA seq-non-specific interaction
    ...
    !   protein-DNA sequence-specific interactions
    if (allocated(ipdpwm2pro_mp)) deallocate (ipdpwm2pro_mp)
    if (allocated(pro_mp2pdpwm_para)) deallocate (pro_mp2pdpwm_para)
    if (allocated(pro_mp2pdpwm_r0_cutoff)) deallocate (pro_mp2pdpwm_r0_cutoff)
    if (allocated(pdpwm_mp_N_ca)) deallocate (pdpwm_mp_N_ca)
    if (allocated(pdpwm_mp_C_ca)) deallocate (pdpwm_mp_C_ca)
    if (allocated(pdpwm_mp_5_base)) deallocate (pdpwm_mp_5_base)
    if (allocated(pdpwm_mp_3_base)) deallocate (pdpwm_mp_3_base)
    if (allocated(pdpwm_para_r0)) deallocate (pdpwm_para_r0)
    if (allocated(pdpwm_para_a530)) deallocate (pdpwm_para_a530)
    if (allocated(pdpwm_para_aNC0)) deallocate (pdpwm_para_aNC0)
    if (allocated(pdpwm_para_aSB0)) deallocate (pdpwm_para_aSB0)
    if (allocated(pdpwm_local_coef)) deallocate (pdpwm_local_coef)
    if (allocated(pdpwm_local_shift)) deallocate (pdpwm_local_shift)
    if (allocated(pdpwm_para_pwm_A)) deallocate (pdpwm_para_pwm_A)
    if (allocated(pdpwm_para_pwm_C)) deallocate (pdpwm_para_pwm_C)
    if (allocated(pdpwm_para_pwm_G)) deallocate (pdpwm_para_pwm_G)
    if (allocated(pdpwm_para_pwm_T)) deallocate (pdpwm_para_pwm_T)
    !   protein-DNA sequence-non-specific interactions
    if (allocated(ipdns2pro_mp)) deallocate (ipdns2pro_mp)
    if (allocated(pro_mp2pdns_para)) deallocate (pro_mp2pdns_para)
    if (allocated(pro_mp2pdns_r0_cutoff)) deallocate (pro_mp2pdns_r0_cutoff)
    if (allocated(pdns_mp_N_ca)) deallocate (pdns_mp_N_ca)
    if (allocated(pdns_mp_C_ca)) deallocate (pdns_mp_C_ca)
    if (allocated(pdns_para_r0)) deallocate (pdns_para_r0)
    if (allocated(pdns_para_aNC0)) deallocate (pdns_para_aNC0)
    if (allocated(pdns_para_aSB0)) deallocate (pdns_para_aSB0)
    if (allocated(pdns_coef)) deallocate (pdns_coef)
