...
! force items
    ...
    integer, parameter :: tm_force_gb = 37 !gb
    integer, parameter :: tm_force_pdpwm = 38 !protein-DNA sequence-specific
    integer, parameter :: tm_force_pdns = 39 !protein-DNA sequence-non-specific
...
! for neighbor list
    ...
    integer, parameter :: tm_neighbor_gb = 58  !gb
...
! for energy
    integer, parameter :: tm_energy_gb = 99   !gb
...
!--------------------------------------------------
    subroutine time_write(lunout)
        ...
        if (inmisc%force_flag(INTERACT%PRO_DNA_PWM)) then
            write (lunout, fmt=fmt1) '_force(pdpwm)   ', total_time(tm_force_pdpwm), trate*total_time(tm_force_pdpwm)
        end if

        if (inmisc%force_flag(INTERACT%PRO_DNA_NONSPEC)) then
            write (lunout, fmt=fmt1) '_force(pdns)   ', total_time(tm_force_pdns), trate*total_time(tm_force_pdns)
        end if
        ...
        if (inmisc%force_flag(INTERACT%GB)) then
            write (lunout, fmt=fmt1) '_force(gb)   ', total_time(tm_force_gb), trate*total_time(tm_force_gb)
        end if
        ...
        if (inmisc%force_flag(INTERACT%GB)) then
            write (lunout, fmt=fmt1) '_neighbor(gb)', total_time(tm_neighbor_gb), trate*total_time(tm_neighbor_gb)
        end if
