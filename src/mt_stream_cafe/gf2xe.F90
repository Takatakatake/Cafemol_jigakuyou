...
!DEC$ ATTRIBUTES FORCEINLINE :: mult_i32
    subroutine mult_i32(a, b, ch, cl)
        ...
        ! To avoid invalid usage of BOZ literals. BOZ literals can be used only
        ! in the `data` statement. (until f95. f03 or later allows us to use it
        ! as an argument of the intrinsic functions.) But `parameter` specifier
        ! requires that the variable should be initialized just after the
        ! declaration, so we need to remove `parameter` specifier to initialize
        ! them in `data` statement.
