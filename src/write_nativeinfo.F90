...
subroutine write_nativeinfo(lunout)
    ...
    use const_physical
    use const_index
    use var_setp, only: inpro, inrna, inmisc, inpdpwm, inpdns
    use var_struct, only: nunit_all, imp2unit, lunit2mp, &
        ...
        ipdns2pro_mp, pro_mp2pdns_para, pdns_para_r0, &     ! PDNS
        pdns_para_aNC0, pdns_para_aSB0, & ! PDNS
        pdns_coef, npdns_pro, npdns_para, & ! PDNS
    ...
    integer :: ibd, iba, idih, icon, ibp, ipdpwm, ipdpwm_para, ipdpwm_tmp
    integer :: ipdns, ipdns_para, ipdns_tmp
    real(PREC) :: dfcontact
   ...
   write (lunout, '(a)') '** coef_aicg13_gauss(kcal/mol) = factor_aicg13 * correct_ba_mgo * coef_aicg13_gauss * energy_unit_protein'
        ...
        if (ndih > 0) then
            ...
            do idih = 1, ndih
                ...
                else if (inmisc%flag_local_unit(iunit1, iunit2, LINTERACT%L_DNA2C)) then
                    write (lunout, '(a5)') dihtype2str()
                else
                    write (lunout, '(a)') ''
                endif
 ...
 write (lunout, '(a)') '** coef_dih_gauss(kcal/mol) = factor_aicg14 * correct_dih_mgo * coef_aicg14(kcal/mol) * energy_unit_protein'
                ...
                do junit = iunit, nunit_all
                    ...
                    do icon = 1, ncon
                        ...
                        if (iunit == iunit1 .and. junit == iunit2) then
                            ...
                            write (lunout, "(a7, 7(1xi6), 2(1xf12.4), (1xi6), (1xf12.4))", ADVANCE='NO') &
                ...
                do junit = iunit, nunit_all
                    ...
                    do ibp = 1, nrna_bp
                        ...
                        if (iunit == iunit1 .and. junit == iunit2) then
                            ...
                            write (lunout, "(a8, 7(1xi6), 2(1xf12.4), (1xi6), (1xf12.4))", ADVANCE='NO') &
        ...
        if (nrna_st > 0) then
            ...
            do icon = 1, nrna_st
                ...
                write (lunout, "(a9, 7(1xi6), 2(1xf12.4), (1xi6), (1xf12.4))", ADVANCE='NO') &
        ...
        ! ------------------------------------------------------------------
        ! write protein-DNA sequence-specific interaction
        if (inmisc%force_flag(INTERACT%PRO_DNA_PWM)) then
            ! write (*,*) " Writing -------------------- PRO_DNA_PWM to native info --------------------"
            ! write (*,*) npdpwm_pro, npdpwm_para
            if (npdpwm_pro > 0) then
                write (lunout, '(a)') '<<<< protein-DNA sequence-specific interaction'
                write (lunout, '(a, i6)') '** total_pdpwm = ', npdpwm_para
                write (lunout, '(a)') ''
                do ipdpwm = 1, npdpwm_pro
                    imp1 = ipdpwm2pro_mp(ipdpwm)
                    iunit1 = imp2unit(imp1)
                    imp1un = imp1 - lunit2mp(1, iunit1) + 1
                    do ipdpwm_tmp = 1, MXMPPDPWM
                        ipdpwm_para = pro_mp2pdpwm_para(ipdpwm_tmp, imp1)
                        if (ipdpwm_para > 0) then
                            write (lunout, "(a5, 2(1xi3), 2(1xi6), 11(1xf8.4))") &
                                'pdpwm', ipdpwm_para, iunit1, imp1, imp1un, &
                                pdpwm_para_r0(ipdpwm_para), pdpwm_para_aSB0(ipdpwm_para)*1.800e2_PREC/F_PI, &
                                pdpwm_para_a530(ipdpwm_para)*1.800e2_PREC/F_PI, pdpwm_para_aNC0(ipdpwm_para)*1.800e2_PREC/F_PI, &
                                pdpwm_para_pwm_A(ipdpwm_para)/inpdpwm%energy_unit_pro_dna_pwm, &
                                pdpwm_para_pwm_C(ipdpwm_para)/inpdpwm%energy_unit_pro_dna_pwm, &
                                pdpwm_para_pwm_G(ipdpwm_para)/inpdpwm%energy_unit_pro_dna_pwm, &
                                pdpwm_para_pwm_T(ipdpwm_para)/inpdpwm%energy_unit_pro_dna_pwm, &
                                pdpwm_local_coef(ipdpwm_para), &
                                pdpwm_local_shift(ipdpwm_para)
                        end if
                    end do
                end do
                write (lunout, '(a4)') '>>>>'
        ...
        ! ------------------------------------------------------------------
        ! write protein-DNA sequence-non-specific interaction
        if (inmisc%force_flag(INTERACT%PRO_DNA_NONSPEC)) then
            ! write (*,*) " Writing -------------------- PRO_DNA_NONSPEC to native info --------------------"
            ! write (*,*) npdns_pro, npdns_para
            if (npdns_pro > 0) then
                write (lunout, '(a)') '<<<< protein-DNA sequence-specific interaction'
                write (lunout, '(a, i6)') '** total_pdns = ', npdns_para
                write (lunout, '(a)') ''
                do ipdns = 1, npdns_pro
                    imp1 = ipdns2pro_mp(ipdns)
                    iunit1 = imp2unit(imp1)
                    imp1un = imp1 - lunit2mp(1, iunit1) + 1
                    do ipdns_tmp = 1, MXMPPDNS
                        ipdns_para = pro_mp2pdns_para(ipdns_tmp, imp1)
                        if (ipdns_para > 0) then
                            write (lunout, "(a4, 4(1xi6), 9(1xf12.4))") &
                                'pdns', ipdns_para, iunit1, imp1, imp1un, &
                                pdns_para_r0(ipdns_para), pdns_para_aNC0(ipdns_para)*1.800e2_PREC/F_PI, &
                                pdns_para_aSB0(ipdns_para)*1.800e2_PREC/F_PI, &
                                pdns_coef(ipdns_para)/inpdns%energy_unit_pro_dna_nonspec
                        end if
                    end do
                end do
                write (lunout, '(a4)') '>>>>'
...
!           angletype2str = ' BSB'
    ...
    character(5) function dihtype2str()
        if (idih2type(idih) == DIHTYPE%RNA_RSPS .OR. idih2type(idih) == DIHTYPE%RNA_YSPS) then
        ...
        else if (idih2type(idih) == DIHTYPE%DNA_PER1) then
            dihtype2str = ' N2P1'
            ...
            WRITE(error_message, "('Error: logical defect in write_native_info (undefined DIHTYPE)',' ',I6,' ',I6,' ',I6)")  imp1,imp2,idih 
            call util_error(ERROR%STOP_ALL, error_message)
        endif
