...
subroutine simu_force_dih(irep, force_mp, force_mp_mgo, ene_unit)
...
!                                           inmgo%nstate_max_mgo, inmgo%nsystem_mgo)
    ...
    real(PREC) :: dcosphi_natdcos, cosphi_nat, dudcos
    real(PREC) :: f1(3), f2(3), f3(3), f4(3)
    real(PREC) :: s21, s24, s31, s34
    real(PREC) :: phi_1, pre1, phi_3, pre4
...
!$OMP&           c12overc22,c23overc22,s21,s24,s31,s34,zahyokei,si_dih, &
!$OMP&           cosphi_nat,dcosphi_natdcos,phi_1,phi_3,dudcos,pre1,pre4, &
!$OMP&           t1norm,t2norm,t3norm,t4norm,t5norm,t6norm,f1,f4,f2,f3, &
!$OMP&           efull,iunit,junit,isys,istat)
    do idih = ksta, kend

        if (coef_dih(1, idih) < ZERO_JUDGE .and. coef_dih(1, idih) < ZERO_JUDGE) cycle
        ...
        ! -------------------------------------------------------------------
        ! calc force
        cosphi_nat = co_dih*dih_cos_nat(idih) + si_dih*dih_sin_nat(idih)
        dcosphi_natdcos = -(dih_cos_nat(idih) - (co_dih/si_dih)*dih_sin_nat(idih))
        phi_1 = coef_dih(1, idih)*dcosphi_natdcos
        phi_3 = coef_dih(2, idih)*3.0*(4.0*cosphi_nat**2 - 1.0)*dcosphi_natdcos

        dudcos = phi_1 + phi_3

!     dudcos =  (coef_dih(1, idih) + coef_dih(2,idih) * (12.0 * cosphi_nat**2 - 3.0) ) &
!              * dcosphi_natdcos
        ...
        if (inmgo%i_multi_mgo == 0) then
            ...
            ! calc energy
            efull = (coef_dih(1, idih)*(1.0e0_PREC - cosphi_nat) + &
                     coef_dih(2, idih)*(1.0e0_PREC - 4.0e0_PREC*cosphi_nat**3 + 3.0e0_PREC*cosphi_nat))
            iunit = imp2unit(imp1)
            junit = imp2unit(imp4)
