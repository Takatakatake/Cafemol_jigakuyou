program aa2cg
    ...
    lunout = iopen_lunnum
    open (lunout, file=cfile_out(1:n - 1), status='REPLACE', iostat=iopen_status)
    ...
    open (lunninfo, file=cfile_ninfo(1:n - 1), status='REPLACE', iostat=iopen_status)
    if (iopen_status > 0) then
