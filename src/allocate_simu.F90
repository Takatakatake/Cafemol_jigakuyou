...
#endif
        replica_energy, &
        sasa, sasa_thread_local
!$  use omp_lib

    implicit none
    integer :: ier = 0, max_threads
...
!$  max_threads = omp_get_max_threads()
    allocate (sasa_thread_local(nmp_all, max_threads), stat=ier)
    if (ier /= 0) call util_error(ERROR%STOP_ALL, error_message)
...
! **********************************************************************
    subroutine check()
        ...
        if (allocated(velo_mp) .OR. &
            ...
            allocated(sasa_thread_local) .OR. &  !sasa
            .FALSE.) then
...
subroutine deallocate_simu()
    ...
    if (allocated(sasa_thread_local)) deallocate (sasa_thread_local)
