...
subroutine setp_native_bond(xyz_mp_init, xyz_dna)
    ...
    use const_physical
    use const_index
    use var_setp, only: inpro, inmisc, indna, indna2, inligand, &
        inrna, indtrna, inarna
            ...
            if (.not. inmisc%flag_local_unit(iunit, iunit, LINTERACT%NOTHING) .and. &
                ...
                do imp = lmp + 1, lunit2mp(2, iunit)
                    ...
                    if (impmod == impmod_S) then
                        ...
                        imp1 = imp - 1
                        imp2 = imp
                        call nat_bond_dna2(ibd, imp1, imp2, BDTYPE%DNA2_PS) !
                        ...
                        if (cmp2seq(imp2) == 'DA ') then
                            call nat_bond_dna2(ibd, imp1, imp2, BDTYPE%DNA2_SA) !
                        else if (cmp2seq(imp2) == 'DT ') then
                            call nat_bond_dna2(ibd, imp1, imp2, BDTYPE%DNA2_ST) !
                        else if (cmp2seq(imp2) == 'DC ') then
                            call nat_bond_dna2(ibd, imp1, imp2, BDTYPE%DNA2_SC) !
                        else if (cmp2seq(imp2) == 'DG ') then
                            call nat_bond_dna2(ibd, imp1, imp2, BDTYPE%DNA2_SG) !
                        else
                        ...
                        imp1 = imp - 2
                        imp2 = imp
                        call nat_bond_dna2(ibd, imp1, imp2, BDTYPE%DNA2_SP) !
...
contains
    ...
    subroutine nat_bond_dna2(ibd, imp1, imp2, i_type_bd)
    ...
    subroutine nat_bond_rna(ibd, iunit, imp1, imp2, cmp, i_type_bd, xyz_bd)
        ...
        case (BDTYPE%RNA_PS)
            ...
            if (inmisc%flag_local_unit(iunit, iunit, LINTERACT%L_DTRNA)) then
                coef_bd(1, ibd) = factor_bd(ibd)*indtrna%cbd_PS
        ...
        case (BDTYPE%RNA_SR)
            ...
            if (inmisc%flag_local_unit(iunit, iunit, LINTERACT%L_DTRNA)) then
                coef_bd(1, ibd) = factor_bd(ibd)*indtrna%cbd_SB
                if (cmp == ' Ab ') then
        ...
        case (BDTYPE%RNA_SP)
            ...
            if (inmisc%flag_local_unit(iunit, iunit, LINTERACT%L_DTRNA)) then
                coef_bd(1, ibd) = factor_bd(ibd)*indtrna%cbd_SP
