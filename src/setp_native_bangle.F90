...
subroutine setp_native_bangle(xyz_mp_init, xyz_dna)
    ...
    use const_physical
    use const_index
    use var_setp, only: inpro, inmisc, indna, indna2, inrna, indtrna, inarna, &
        inligand
            ...
            if (inmisc%flag_local_unit(iunit, iunit, LINTERACT%L_DNA2) .OR. &
                ...
                do imp = lmp + 2, lunit2mp(2, iunit)
                    ...
                    if (impmod == impmod_S) then
                        ...
                        imp2 = imp - 1
                        imp3 = imp
                        call nat_bangle_dna2(iba, imp1, imp2, imp3, BATYPE%DNA2_SPS)
                        ...
                        imp3 = imp
                        if (cmp2seq(imp3) == 'DA ') then
                            call nat_bangle_dna2(iba, imp1, imp2, imp3, BATYPE%DNA2_PSA)
                        else if (cmp2seq(imp3) == 'DT ') then
                            call nat_bangle_dna2(iba, imp1, imp2, imp3, BATYPE%DNA2_PST)
                        else if (cmp2seq(imp3) == 'DC ') then
                            call nat_bangle_dna2(iba, imp1, imp2, imp3, BATYPE%DNA2_PSC)
                        else if (cmp2seq(imp3) == 'DG ') then
                            call nat_bangle_dna2(iba, imp1, imp2, imp3, BATYPE%DNA2_PSG)
                        else
                        ...
                        imp3 = imp
                        if (cmp2seq(imp1) == 'DA ') then
                            call nat_bangle_dna2(iba, imp1, imp2, imp3, BATYPE%DNA2_ASP)
                        else if (cmp2seq(imp1) == 'DT ') then
                            call nat_bangle_dna2(iba, imp1, imp2, imp3, BATYPE%DNA2_TSP)
                        else if (cmp2seq(imp1) == 'DC ') then
                            call nat_bangle_dna2(iba, imp1, imp2, imp3, BATYPE%DNA2_CSP)
                        else if (cmp2seq(imp1) == 'DG ') then
                            call nat_bangle_dna2(iba, imp1, imp2, imp3, BATYPE%DNA2_GSP)
                        else
                        ...
                        imp2 = imp - 2
                        imp3 = imp
                        call nat_bangle_dna2(iba, imp1, imp2, imp3, BATYPE%DNA2_PSP)
...
contains
    ...
    subroutine nat_bangle_dna2(iba, imp1, imp2, imp3, i_type_ba)
    ...
    subroutine nat_bangle_rna(iba, iunit, imp1, imp2, imp3, cmp, i_type_ba, xyz_ba)
        ...
        if (inmisc%flag_local_unit(iunit, iunit, LINTERACT%L_DTRNA)) then
            select case (i_type_ba)
            case (BATYPE%RNA_RSP)
                coef_ba(1, iba) = factor_ba(iba)*indtrna%cba_BSP
                if (cmp == ' Ab ') then
            ...
            case (BATYPE%RNA_PSP)
                coef_ba(1, iba) = factor_ba(iba)*indtrna%cba_PSP
                ba_nat(iba) = inarna%angl_PSP
            case (BATYPE%RNA_SPS)
                coef_ba(1, iba) = factor_ba(iba)*indtrna%cba_SPS
                ba_nat(iba) = inarna%angl_SPS
            case (BATYPE%RNA_PSR)
                coef_ba(1, iba) = factor_ba(iba)*indtrna%cba_PSB
                if (cmp == ' Ab ') then
            ...
            case (BATYPE%RNA_PSY)
                ...
                !case (BATYPE%RNA_BSB)
                !    coef_ba(1, iba) = factor_ba(iba) * indtrna%cba_BSB
            case default
                error_message = 'Error: logical defect in setp_native_bangle'
                call util_error(ERROR%STOP_ALL, error_message)
            end select
