...
subroutine setp_native_go(xyz_mp_init, &
    ...
    use var_struct, only: nunit_all, nmp_all, imp2unit, cmp2atom, &
...
!  integer :: icalc_1210, icalc_morse, icalc_basepair, icalc_enm
    ...
    logical :: flag_AICG1, flag_AICG2, flag_AICG2_PLUS   !AICG
!  integer :: idel, ini, las, ini2, las2
    ...
    do imp = 1, nmp_all - 1
        ...
        if (inmisc%i_go_atom_dna == 1 .and. (iclass_unit(iunit) == CLASS%DNA .or. iclass_unit(iunit) == CLASS%DNA2)) then
            if (cmp2atom(imp) (2:2) == 'O' .or. cmp2atom(imp) (2:2) == 'P') then
                lmp2con(imp) = icon
...
!        icalc_basepair = mod(inmisc%itype_nlocal(iunit, junit), INTERACT%PAIR_RNA)
            ...
            flag_AICG2_PLUS = inmisc%flag_nlocal_unit(iunit, junit, INTERACT%AICG2_PLUS) !nonlocal aicg2plus
            !--------------------------------
...
!            icalc_morse /= 0 .AND. icalc_basepair /= 0 ) then
            if (.not. flag_enm .AND. .not. flag_1210 .AND. &
                ...
                .not. flag_AICG1 .AND. .not. flag_AICG2 .AND. .not. flag_AICG2_PLUS) then !nonlocal aicg2plus
                cycle jmp_loop
            end if
...
!        end if
            ...
            if (inmisc%i_go_atom_dna == 1 .and. (iclass_unit(junit) == CLASS%DNA .or. iclass_unit(junit) == CLASS%DNA2)) then
                if (cmp2atom(jmp) (2:2) == 'O' .or. cmp2atom(jmp) (2:2) == 'P') cycle jmp_loop
            end if
...
!        else if (flag_1210 .AND. dist2 < dfcontact2) then  !AICG
            else if ((flag_1210 .OR. flag_AICG1 .OR. flag_AICG2 .OR. flag_AICG2_PLUS) &  !nonlocal aicg2plus
                     .AND. dist2 < dfcontact2) then  !AICG
