...
subroutine inp_datafile()
    ...
    if (myrank == 0) then
        ...
        if (inmisc%force_flag_local(LINTERACT%L_AICG2) .OR. &
            ...
            inmisc%force_flag(INTERACT%AICG2) .OR. &
            inmisc%force_flag(INTERACT%AICG2_PLUS)) then !nonlocal aicg2plus
            if (i_aicg == AICG%USER) then
                n = index(path_aicg, ' ')
                if (path_aicg /= '') then
        ...
        ! --------------------------------------------------------------------
        ! protein-DNA sequence-specific interaction parameters
        if (path_para /= '') then
            filename_para(14) = path_para(1:l - 1)//'/'//'pro_dna_pwm.para'
        else
            filename_para(14) = './para/pro_dna_pwm.para'
        end if
        write (*, '(a50,i3,a3,a)') "open protein-DNA sequence-specific parameter file(", infile%para_pdpwm, &
            "): ", trim(filename_para(14))
        open (infile%para_pdpwm, file=filename_para(14), status='old', action='read', &
              iostat=iopen_status)
        if (iopen_status > 0) then
            error_message = 'Error: cannot open the file: '//filename_para(14)
            call util_error(ERROR%STOP_ALL, error_message)
        end if

        ! --------------------------------------------------------------------
        ! protein-DNA sequence-non-specific interaction parameters
        if (path_para /= '') then
            filename_para(15) = path_para(1:l - 1)//'/'//'pro_dna_nonspec.para'
        else
            filename_para(15) = './para/pro_dna_nonspec.para'
        end if
        write (*, '(a54,i3,a3,a)') "open protein-DNA sequence-non-specific parameter file(", infile%para_pdns, &
            "): ", trim(filename_para(15))
        open (infile%para_pdns, file=filename_para(15), status='old', action='read', &
              iostat=iopen_status)
        if (iopen_status > 0) then
            error_message = 'Error: cannot open the file: '//filename_para(15)
            call util_error(ERROR%STOP_ALL, error_message)
        end if
