...
subroutine simu_collision_mpc4(irep)
...
!  use if_energy
    ...
    use var_simu, only: istep, velo_mp
    ...
    if (myrank == 0) then
        ...
        TIME_S(tm_velo_mpc)
        call simu_stream_mpc4(velo_mp, irep)
        TIME_E(tm_velo_mpc)
...
!  end do
        ...
        TIME_S(tm_grid_mpc)
        call simu_grid_devision_mpc4(irep)
        TIME_E(tm_grid_mpc)
...
!  call simu_rotate_velo_mpc2(velo_mp, irep)
        call simu_rotate_velo_mpc_rev4(velo_mp, irep)
...
end subroutine simu_collision_mpc4
