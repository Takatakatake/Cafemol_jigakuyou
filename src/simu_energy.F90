...
!$  tn = omp_get_thread_num()
    ...
    if (inmisc%force_flag(INTERACT%DTRNA)) then
        ...
        call simu_energy_dtrna_hbond(irep, pnle_unit_l(:, :, :, tn), pnlet_l(:, tn))
    endif
...
!$omp end master
    ...
    if (inmisc%force_flag(INTERACT%PRO_DNA_PWM)) then
        call simu_energy_pro_dna_pwm(irep, pnle_unit_l(:, :, :, tn), pnlet_l(:, tn))
    endif

    if (inmisc%force_flag(INTERACT%PRO_DNA_NONSPEC)) then
        call simu_energy_pro_dna_nonspec(irep, pnle_unit_l(:, :, :, tn), pnlet_l(:, tn))
    endif
...
!gb
    if (inmisc%force_flag(INTERACT%GB)) then
        TIME_S(tm_energy_gb)
        call simu_energy_gb(irep, pnlet_l(:, tn))
        TIME_E(tm_energy_gb)
    endif

    if (inmisc%i_stage == 1) then
        call simu_energy_stage(irep, pnlet_l(:, tn))
    end if
...
!$omp end parallel
    ...
    if (inmisc%i_afm_fitting == 1) then
        call simu_energy_afm_fitting(irep, pnlet)
    end if
    ...
    if (inmisc%i_winz == 1) then
        call simu_energy_winz(irep, pnlet)
    end if
