...
#include "format.F90"
    ...
    if (myrank == 0) then
        ...
        ! File type
        write (lunout, '(a)', ADVANCE='NO') 'PSF'
        ! File title
        write (lunout, '(a)') ' CHARMM'
        write (lunout, '(a)') ''

        ! Remarks
        write (lunout, '(i8)', ADVANCE='NO') 3
        write (lunout, '(a)') ' !NTITLE'
        write (lunout, '(a)') '* Molecular Dynamics Simulation'
        write (lunout, '(a)') '* Code : CafeMol ver. 3.2'
        write (lunout, '(a)') '* Developed by Takada Lab, Kyoto University'
        write (lunout, '(a)') ''
