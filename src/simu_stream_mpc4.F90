...
subroutine simu_stream_mpc4(velo_mp, irep)
    ...
    use time, only: time_s, time_e, &
        tm_velo_mpc01, tm_velo_mpc02, tm_velo_mpc03, tm_velo_mpc04, tm_velo_mpc05, &
        tm_velo_mpc06, tm_velo_mpc07, tm_velo_mpc08, tm_velo_mpc09, tm_velo_mpc10
#endif
...
!     cmass_nh(1:ncs) = 100000.0
    ...
    TIME_S(tm_velo_mpc01)
!$omp parallel
!$omp do reduction(+:P_total_mpc,total_mass_mpc)
    do is = 1, n_solv
...
!$omp end do
!$omp end parallel
    TIME_E(tm_velo_mpc01)

    TIME_S(tm_velo_mpc02)
!!$omp parallel
!!$omp do reduction(+:P_total_mpc,total_mass_mpc)
    do imp = 1, nmp_real
...
!$omp end do
!$omp end parallel
    TIME_E(tm_velo_mpc03)

    TIME_S(tm_velo_mpc04)
!!$omp parallel
!!$omp do
    do imp = 1, nmp_real
        if (ifix_mp(imp) == 1) cycle
...
!!$omp end parallel
    ...
    TIME_S(tm_velo_mpc05)
    !!<<for mpc solvent particle>>
    n_solv = inmpc%n_all_solv
!!$omp parallel
!!$omp do reduction(+:total_kine_E2_All)
    do is = 1, n_solv
...
!$omp end parallel
        ...
        TIME_E(tm_velo_mpc08)

        xtrans = xtrans*tstep

        TIME_S(tm_velo_mpc09)
!$omp parallel
!$omp do
        do is = 1, n_solv
...
!$omp end do
!$omp end parallel
        TIME_E(tm_velo_mpc09)

        TIME_S(tm_velo_mpc10)
!$omp parallel
!$omp do
        do is = 1, n_solv
...
!$omp end do
!$omp end parallel
        TIME_E(tm_velo_mpc10)
    end if

end subroutine simu_stream_mpc4
